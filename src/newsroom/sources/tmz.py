"""TMZ (celebrity news RSS) ingestion source.

Reads TMZ's public RSS feed and upserts recent posts. We fetch the bytes with
httpx (sending ``settings.http_user_agent``) and parse with feedparser off the
event loop, exactly like ``reddit_rss.py``.
